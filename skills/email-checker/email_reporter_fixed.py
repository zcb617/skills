...
class EmailReporter:
    ...
    def send_notification(self, message):
        ...
        # 发送钉钉通知
        try:
            # 使用更安全的方法调用OpenClaw，不指定target，因为钉钉是DM策略
            cmd_dingtalk = [
                "bash", "-c",
                f'source /etc/profile && source /home/zhangcb/.nvm/nvm.sh && cd /home/zhangcb/.nvm/versions/node/v24.13.0/lib/node_modules/openclaw && openclaw message send --channel dingtalk --target "小张同学" --message \'{message[:2000]}\''
            ]
        ...
        try:
            cmd_whatsapp = [
                "bash", "-c",
                f'source /etc/profile && source /home/zhangcb/.nvm/nvm.sh && cd /home/zhangcb/.nvm/versions/node/v24.13.0/lib/node_modules/openclaw && openclaw message send --channel whatsapp --target "+8618605738770" --message \'$(echo "{message[:2000]}" | sed "s/[$]/\\$/g")\''
            ]
