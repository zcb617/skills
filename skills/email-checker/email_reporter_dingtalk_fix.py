...
class EmailReporter:
    ...
    def send_notification(self, message):
        ...
        # 发送钉钉通知 - 尝试使用钉钉插件的默认行为
        # 由于钉钉配置中dmPolicy为"open"，可能需要通过其他方式发送
        try:
            # 先尝试使用钉钉的机器人API方式发送
            import requests
            import time
            ...
            # 从配置获取钉钉信息
            with open('/home/zhangcb/.openclaw/openclaw.json', 'r') as f:
                config = json.load(f)
            
            # 提取钉钉凭证
            client_id = config['channels']['dingtalk']['clientId']
            client_secret = config['channels']['dingtalk']['clientSecret']
            
            print(f"钉钉客户端ID: {client_id}")
            print("注意：由于钉钉配置复杂性，需要通过OpenClaw内部机制发送消息")
            print("钉钉插件配置显示dmPolicy为'open'，应能自动发送到默认用户")
            
        except Exception as e:
            print(f"处理钉钉配置时出错: {str(e)}")
        
        # 发送WhatsApp通知
        try:
            # 直接调用OpenClaw的message命令发送WhatsApp消息
            cmd_whatsapp = [
                "bash", "-c",
