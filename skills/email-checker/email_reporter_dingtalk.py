...
class EmailReporter:
    ...
    def send_notification(self, message):
        """发送邮件简报通知到钉钉和WhatsApp"""
        print(f"准备发送邮件简报:\\n{message[:500]}...")  # 只打印前500个字符
        
        # 发送钉钉通知 - 由于钉钉配置中dmPolicy为"open"，应该可以直接发送
        try:
            # 使用OpenClaw内置的消息发送功能
            # 根据钉钉配置，dmPolicy设为"open"意味着可以直接发送私信
            cmd_dingtalk = [
                "bash", "-c",
                f'source /etc/profile && source /home/zhangcb/.nvm/nvm.sh && cd /home/zhangcb/.nvm/versions/node/v24.13.0/lib/node_modules/openclaw && echo \'{message[:2000]}\' | openclaw message send --channel dingtalk'
            ]
            result_dingtalk = subprocess.run(cmd_dingtalk, capture_output=True, text=True, timeout=30)
            if result_dingtalk.returncode == 0:
                print("钉钉邮件简报发送成功")
            else:
                print(f"钉钉邮件简报发送失败: {result_dingtalk.stderr}")
                
                # 尝试另一种方式，如果上面的方式不行
                # 因为钉钉机器人的配置可能需要使用特定的API方式
                print("尝试使用钉钉机器人API方式发送...")
                # 这里可以使用钉钉的Webhook API，但需要配置webhook URL
                # 目前我们使用的是应用级别的配置，可能需要指定用户ID
                
        except Exception as e:
            print(f"发送钉钉邮件简报时出错: {str(e)}")
        
        # 发送WhatsApp通知
        try:
            cmd_whatsapp = [
                "bash", "-c",
                f'source /etc/profile && source /home/zhangcb/.nvm/nvm.sh && cd /home/zhangcb/.nvm/versions/node/v24.13.0/lib/node_modules/openclaw && echo \'{message[:2000]}\' | openclaw message send --channel whatsapp --target "+8618605738770"'
            ]
